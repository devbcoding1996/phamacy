...
class DrugType extends Database
{
  ...
  {
    try {
      $stm = $this->pdo->prepare("SELECT * FROM drug_type ORDER BY id DESC");
      $stm->execute();
      if($stm->rowCount() > 0) {
  ...
  {
    try {
      $sql = "INSERT INTO drug_type (`name`) VALUES (:name)";
  ...
  {
    try {
      $stmt = $this->pdo->prepare("SELECT * FROM drug_type WHERE id = :id");
      // Bind the parameters
  ...
  public function update($data) 
  {
    try {
      ...
      $sql = "UPDATE drug_type SET `name` = :name WHERE id = :id";
      $stmt = $this->pdo->prepare($sql);
  ...
  {
    try {
      $stmt = $this->pdo->prepare("DELETE FROM drug_type WHERE id = :id");
      // Bind the parameters
      $stmt->bindParam(':id', $data[0]);
